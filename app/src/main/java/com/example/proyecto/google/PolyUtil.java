...
import com.example.proyecto.Muestra;
...
import static com.example.proyecto.google.MathUtil.*;
...
public class PolyUtil {
    ...
    public static List<Muestra> locationIndexOnEdgeOrPathPoint(LatLng point, List<LatLng> poly, boolean closed,
                                                boolean geodesic, double toleranceEarth) {
        int size = poly.size();
        Muestra before = new Muestra();
        ...
        if (geodesic) {
            for (LatLng point2 : poly) {
                ...
                if (isOnSegmentGC(lat1, lng1, lat2, lng2, lat3, lng3, havTolerance)) {
                    //return
                    before.setPoint(poly.get(Math.max(0, idx - 1)-1));
                    before.setIndex(Math.max(0, idx - 1)-1);
                    if(list.contains(before))
                    {
                        if(SphericalUtil.computeDistanceBetween(poly.get(Math.max(0,idx-1)-1),point)>SphericalUtil.computeDistanceBetween(poly.get(Math.max(0,idx-1)),point))
                        {
                            int ind= list.indexOf(before);
                            list.remove(ind);
                            list.add(new Muestra(poly.get(Math.max(0, idx - 1)),Math.max(0, idx - 1)));
                        }
                    }
                    else
                    {
                        list.add(new Muestra(poly.get(Math.max(0, idx - 1)),Math.max(0, idx - 1)));//Math.max(0, idx - 1);
                    }
    ...
    public static int locationIndexOnEdgeOrPath(LatLng point, List<LatLng> poly, boolean closed,
        ...
        if (geodesic) {
            for (LatLng point2 : poly) {
                ...
                double lng2 = toRadians(point2.longitude);
                if (isOnSegmentGC(lat1, lng1, lat2, lng2, lat3, lng3, havTolerance)) {
                    return Math.max(0, idx - 1);//Math.max(0, idx - 1);
                }
                lat1 = lat2;
                lng1 = lng2;
                idx++;
